...
mkdir cuda_opt_cc86 && cd cuda_opt_cc86
cmake ../kokkos \
...
mkdir cuda_opt_cc86 && cd cuda_opt_cc86
cmake ../mfem -DMFEM_USE_CUDA=YES -DMFEM_USE_MPI=YES -DCUDA_ARCH=sm_86 -DMETIS_DIR=../../metis-5.1.0/build/Linux-x86_64/install -DHYPRE_DIR=../../hypre_dev/hypre/src/hypre -DCMAKE_INSTALL_PREFIX=install
make -j64
make install
cd ../..
...
cmake ../pumiMBBL \
-DCMAKE_INSTALL_PREFIX=install \
-DKokkos_ROOT=../../Kokkos/cuda_opt_cc86/install
make -j64
make install
cd ../..
...
-DMFEM_ROOT=../../mfem_dev/cuda_opt_cc86/install \
-DPUMIMBBL_ROOT=../../pumiMBBL_dev/cuda_opt_cc86/install \
